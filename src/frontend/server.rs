...
pub struct FrontendService {
    search_client: Arc<Mutex<SearchClient<Channel>>>,
    profile_client: Arc<Mutex<ProfileClient<Channel>>>,
...
impl FrontendService {
    pub fn new(search: SearchClient<Channel>, profile: ProfileClient<Channel>, log_path: Option<PathBuf>) -> Self {
        let mut tracer = Tracer::new();
        tracer.new_end_to_end_entry("search");
        tracer.new_end_to_end_entry("profile");
        FrontendService {
            search_client: Arc::new(Mutex::new(search)),
            profile_client: Arc::new(Mutex::new(profile)),
            log_path,
...
impl FrontendService {
    async fn handle_search(&self, request: Request<Body>) -> Result<Response<Body>> {
        ...
        let result = self.search_client.lock().await.nearby(search_req).await?;
        self.tracer
            .borrow_mut()
        ...
        let start = Instant::now();
        let result = self.profile_client.lock().await.get_profiles(profile_req).await?;
        self.tracer
            .borrow_mut()
            .record_end_to_end("profile", start.elapsed())?;
        log::trace!("searchHandler gets profileResp");

        let response_json = geo_json_response(result.get_ref())?;
...
fn geo_json_response<'s>(res: &ProfileResult) -> Result<String> {
