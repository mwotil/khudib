...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let search_client = SearchClient::connect(format!("http://{}:{}", args.search_addr, args.search_port)).await?;
    let profile_client = ProfileClient::connect(format!("http://{}:{}", args.profile_addr, args.profile_port)).await?;
