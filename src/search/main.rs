...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let geo_client = GeoClient::connect(format!("http://{}:{}", args.geo_addr, args.geo_port)).await?;
    log::info!("Connecting to rate server...");
    let rate_client = RateClient::connect(format!("http://{}:{}", args.rate_addr, args.rate_port)).await?;
