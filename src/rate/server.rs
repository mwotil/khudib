...
impl From<db::RatePlan> for RatePlan {
    fn from(plan: db::RatePlan) -> Self {
        RatePlan {
            ...
            code: plan.code,
            in_date: plan.in_date,
            out_date: plan.out_date,
...
impl Rate for RateService {
    ...
    ) -> Result<Response<RateResult>, tonic::Status> {
...
impl RateService {
    async fn get_rates_internal<'s>(&self, request: RateRequest) -> Result<RateResult> {
